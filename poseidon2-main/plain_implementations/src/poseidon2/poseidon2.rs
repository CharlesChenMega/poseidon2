...
impl<F: PrimeField> Poseidon2<F> {
    ...
    pub fn hash_for_shitable(&self, input: &[F]) -> Vec<u8> {
    ...
    fn matmul_m4(&self, input: &mut[F]) {
        ...
        for i in 0..t4 {
            let start_index = i * 4;
            let mut t_0 = input[start_index];//t0=I0+I1
            t_0.add_assign(&input[start_index + 1]);
            let mut t_1 = input[start_index + 2];//t1=I2+I3
            t_1.add_assign(&input[start_index + 3]);
            let mut t_2 = input[start_index + 1];//t2=I1^2+I2+I3
            t_2.double_in_place();
            t_2.add_assign(&t_1);
            let mut t_3 = input[start_index + 3];//t3=I3^2+I0+I1
            t_3.double_in_place();
            t_3.add_assign(&t_0);
            let mut t_4 = t_1;//t4=(I2+I3)^4+I3^2+I0+I1
            t_4.double_in_place();
            t_4.double_in_place();
            t_4.add_assign(&t_3);
            let mut t_5 = t_0;//t5=(I0+I1)^4+I1^2+I2+I3
            t_5.double_in_place();
            t_5.double_in_place();
            t_5.add_assign(&t_2);
            let mut t_6 = t_3;//t6=I3^2+I0+I1+(I0+I1)^4+I1^2+I2+I3
            t_6.add_assign(&t_5);
            let mut t_7 = t_2;
            t_7.add_assign(&t_4);
    ...
    fn matmul_internal(&self, input: &mut[F], mat_internal_diag_m_1: &[F]) {
        ...
        match t {
            2 => {
                // [2, 1],y0=2x0+x1,y0=[x0,x1].[2, 1]
                // [1, 3],y1=x0+3x1,y1=[x0,x1].[1, 3]
                let mut sum = input[0];
                sum.add_assign(&input[1]);
                input[0].add_assign(&sum);
            ...
            3 => {
                // [2, 1, 1],y0=2x0+x1+x2,y0=[x0,x1,x2].[2, 1, 1]
                // [1, 2, 1],y1=x0+2x1+x2,y1=[x0,x1,x2].[1, 2, 1]
                // [1, 1, 3],y2=x0+x1+3x2,y2=[x0,x1,x2].[1, 1, 3]
                let mut sum = input[0];
                sum.add_assign(&input[1]);
                sum.add_assign(&input[2]);
                input[0].add_assign(&sum);
...
mod poseidon2_tests_babybear {
    ...
    use crate::poseidon2::poseidon2_instance_goldilocks::POSEIDON2_GOLDILOCKS_8_PARAMS;
